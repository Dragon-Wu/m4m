...
def list_to_txt_nested(list_nested, path_saved):
    """
    list_to_txt(list_saved, path_saved)
    """
    with open(path_saved, "w") as f:
...
def list_clean_blank_short(list_processed, len_min=5):
    ...
    return list(
        filter(
            lambda x: isinstance(x, str) and len(
                x.split()) >= len_min, list_processed
        )
    )
...
def list_clean_blank(list_processed):
    ...
    """
    return list(
        filter(lambda x: isinstance(x, str) and len(
            x.split()) > 0, list_processed)
    )


def ListArray_2_FlattenArray(ListArray):
    '''
    input [array, array, ...,array]
    return a integrated array
    '''
    for idx, array in enumerate(ListArray):
        FlattenArray = array if idx == 0 else np.append(FlattenArray, array)
    # print(type(FlattenArray), FlattenArray.shape)
    return FlattenArray


def seed_everything(seed, torch=None):
    seed = int(seed)
    random.seed(seed)
...
def cal_CI(data, confidence=0.95):
    a = 1.0 * np.array(data)
    n = len(a)
    mean, std, se = np.mean(a), np.std(a), scipy.stats.sem(a)
